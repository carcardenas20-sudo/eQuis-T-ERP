...
async function runMigrations(client) {
  const migrations = [
    ...
    {
      name: 'add_payment_typed_cols_v2',
      sql: async () => {
        // Ensure all typed columns exist (in case table was created before schema had them)
        await client.query(`ALTER TABLE entity_payment ADD COLUMN IF NOT EXISTS employee_id TEXT`);
        await client.query(`ALTER TABLE entity_payment ADD COLUMN IF NOT EXISTS payment_date TIMESTAMPTZ`);
        await client.query(`ALTER TABLE entity_payment ADD COLUMN IF NOT EXISTS payment_type TEXT`);
        await client.query(`ALTER TABLE entity_payment ADD COLUMN IF NOT EXISTS amount NUMERIC(14,2)`);
        await client.query(`ALTER TABLE entity_payment ADD COLUMN IF NOT EXISTS status TEXT`);
        await client.query(`ALTER TABLE entity_payment ADD COLUMN IF NOT EXISTS type TEXT`);
        await client.query(`ALTER TABLE entity_payment ADD COLUMN IF NOT EXISTS location_id TEXT`);
        // Create indexes
        await client.query(`CREATE INDEX IF NOT EXISTS idx_entity_payment_type ON entity_payment(type)`);
        await client.query(`CREATE INDEX IF NOT EXISTS idx_entity_payment_location_id ON entity_payment(location_id)`);
        await client.query(`CREATE INDEX IF NOT EXISTS idx_entity_payment_employee_id ON entity_payment(employee_id)`);
        await client.query(`CREATE INDEX IF NOT EXISTS idx_entity_payment_payment_date ON entity_payment(payment_date)`);
        await client.query(`CREATE INDEX IF NOT EXISTS idx_entity_payment_status ON entity_payment(status)`);
        // Populate typed columns from JSONB data for existing records
        await client.query(`
          UPDATE entity_payment SET
            type = NULLIF(data->>'type', ''),
            location_id = NULLIF(data->>'location_id', ''),
            amount = CASE WHEN amount IS NULL THEN NULLIF(data->>'amount', '')::NUMERIC ELSE amount END,
            payment_date = CASE WHEN payment_date IS NULL THEN NULLIF(data->>'payment_date', '')::TIMESTAMPTZ ELSE payment_date END
          WHERE type IS NULL OR location_id IS NULL OR amount IS NULL OR payment_date IS NULL
        `);
        console.log('✅ Migration: entity_payment typed columns ensured and populated');
      }
    },
    {
